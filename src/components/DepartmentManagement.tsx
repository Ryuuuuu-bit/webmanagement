...
export default function DepartmentManagement({
  ...
  function onDelete(id: string, name: string) {
    if (!confirm(`ลบสาขาวิชา "${name}" ใช่ไหม?`)) return;
    startTransition(async () => {
          ...
          editingId === d.id ? (
            ...
            <div key={d.id} className="flex flex-wrap items-center justify-between gap-2 border-t border-line-soft pt-2 text-sm first:border-t-0 first:pt-0">
              ...
              <div className="flex items-center gap-3">
                ...
                <button disabled={pending} onClick={() => onDelete(d.id, d.name)} className="text-xs font-semibold text-danger disabled:opacity-40">ลบ</button>
              </div>
